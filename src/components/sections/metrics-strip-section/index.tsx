...
export default function MetricsStripSection({
  ...
  return (
    <section
      id="proof-metrics"
      aria-label={heading || "Verified proof metrics"}
      className={cn(
      ...
      <div className="container">
        ...
        >
          {visible.map((metric) => (
            ...
            >
              <dt className="text-muted-foreground order-2 text-xs uppercase tracking-wider">
                {metric.label}
              </dt>
              <dd className="text-primary order-1 font-display text-4xl font-bold leading-none tracking-tight">
                {metric.value}
              </dd>
              {metric.verification === "verified" && (
                <span
                  className="text-muted-foreground/60 order-3 text-[10px] uppercase tracking-widest"
                  aria-label="Verified"
                >
                  ✓ verified
                </span>
